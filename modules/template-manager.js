...
(function () {
  ...
  class TemplateManager {
    constructor() {
      ...
      this.BUILT_IN_PATHS = Object.freeze([
        "/templates/wjx.json",
        "/templates/tencent.json",
        "/templates/jnlab.json",
        "/templates/moodle.json",
        "/templates/canvas-classic.json",
        "/templates/google-forms.json",
        "/templates/surveyjs.json",
        "/templates/open-edx.json",
      ]);
    }
    ...
    async loadBuiltInTemplates() {
      this.builtInTemplates = [];
      for (const path of this.BUILT_IN_PATHS) {
        const template = await this._fetchTemplate(path);
        if (!template) continue;
        this.builtInTemplates.push(template);
        window.siteMatcher.registerTemplate(template);
        console.log(`[TemplateManager] 内置模板加载成功: ${template.siteName}`);
      }
    }
