  ...
  class EnhancedScanner {
    ...
    _parseQuestionWithTemplate(element, index, template) {
      ...
      if (selectors.questionTypes.detectByAttribute) {
        ...
        for (const [type, config] of Object.entries(questionTypes)) {
          if (!['single', 'multiple', 'fill'].includes(type)) continue;
      ...
      } else if (selectors.questionTypes.detectByInputType) {
        // 通过题目容器内部的 input 类型判断题型，适合 Vue/Element UI 页面。
        for (const [type, config] of Object.entries(questionTypes)) {
          if (!['single', 'multiple', 'fill'].includes(type)) continue;
          if (
            config.inputSelector &&
            element.querySelector(config.inputSelector)
          ) {
            questionType = type;
            typeSelectors = config;
            break;
      ...
      // Question media, excluding the site's decorative selection icons.
      const mediaSelector = 'img:not(.selectImg):not([aria-hidden="true"]), canvas, video, audio, object, iframe';
      question.hasUnsupportedMedia = Boolean(titleElement?.querySelector(mediaSelector));

      // 根据类型提取选项或输入框
      if (questionType === "single" || questionType === "multiple") {
        question.options = this._parseOptions(element, typeSelectors, template);
        question.hasUnsupportedMedia ||= Array.from(element.querySelectorAll(typeSelectors.optionItem))
          .some(option => Boolean(option.querySelector(mediaSelector)));
    ...
    _parseOptions(container, typeSelectors, template) {
      ...
      optionElements.forEach((optionEl, idx) => {
        ...
        // 生成精确选择器
        const selector = this._generateSelector(input, template, container, idx);
    ...
    _parseInputs(container, typeSelectors) {
      ...
      inputElements.forEach((input, index) => {
        const selector = this._generateSelector(input, null, container, index);
        inputs.push({
          selector: selector,
          element: input,
        });
     ...
     * @private
     */
    _generateSelector(element, template, container = null, position = 0) {
      // 优先使用题目容器 ID，避免 input[value="A"] 在全页重复导致定位到第一题。
      if (container && container.id) {
        const tagName = element.tagName.toLowerCase();
        const typeSelector = element.type
          ? `${tagName}[type="${element.type}"]`
          : tagName;
        const value = element.getAttribute("value");
        if (value !== null) {
          const escapedValue = value.replace(/(["\\])/g, "\\$1");
          return `#${container.id} ${typeSelector}[value="${escapedValue}"]`;
        }
        return `#${container.id} ${typeSelector}:nth-of-type(${position + 1})`;
      }
