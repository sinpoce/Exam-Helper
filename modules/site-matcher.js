...
(function() {
  ...
  class SiteMatcher {
    ...
    matchTemplate(url = window.location.href) {
      ...
      // 优先使用 URL；对于 SurveyJS 这类可嵌入任意站点的组件，再用多重
      // DOM 特征识别。单个宽泛类名永远不足以触发模板。
      if (typeof document !== 'undefined') {
        for (const [siteId, template] of this.templates) {
          if (this._matchDomSignatures(template.domSignatures, document)) {
            console.log(`[SiteMatcher] DOM特征匹配成功: ${siteId}`);
            return template;
          }
        }
      }
    ...
    _matchDomSignatures(signatures, root) {
      if (!Array.isArray(signatures) || !root?.querySelector) return false;
      const has = selector => {
        try { return Boolean(root.querySelector(selector)); } catch (_) { return false; }
      };
      return signatures.some(signature => {
        const rule = typeof signature === 'string' ? { all: [signature] } : signature;
        const all = Array.isArray(rule?.all) ? rule.all.filter(Boolean) : [];
        const any = Array.isArray(rule?.any) ? rule.any.filter(Boolean) : [];
        return all.length > 0 && all.every(has) && (!any.length || any.some(has));
      });
    }
