  ...
  function createTemplateCard(template) {
    ...
    card.innerHTML = `
      ...
      <div class="template-urls">
        ${
          template.urlPatterns?.length
            ? template.urlPatterns
                .map((url) => `<span class="url-tag">${url}</span>`)
                .join("")
            : template.domSignatures?.length
              ? '<span class="url-tag">嵌入式组件 · DOM 自动识别</span>'
              : '<span class="url-tag">正则路径匹配</span>'
        }
        ${template.sourceReferences?.length ? `<span class="url-tag">${template.sourceReferences.length} 个上游参考</span>` : ""}
      </div>
    `;
